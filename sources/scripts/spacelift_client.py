...
def check_vcs_exists(vcs_id: str) -> bool:
    ...
    try:
        ...
        return result.get("repo") is not None
    except SpaceliftError:
        return False
...
def get_vcs_by_name(name: str, space_id: str) -> Optional[str]:
    ...
    query = """
    query ListRepos($spaceID: ID!) {
        repos(spaceID: $spaceID, first: 50) {
            edges {
                node {
                    id
                    name
    ...
    edges = result.get("repos", {}).get("edges", [])
...
def commit_files(
    ...
    try:
        ...
        commit_sha = result["revisionCreate"]["sha"]
